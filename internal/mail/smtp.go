...
// OperationTimeout is the max time for SMTP dial and the full send (connect, TLS, auth, delivery).
...
func (s *Sender) Send(to, subject, body string) error {
	return s.sendMIME(to, subject, "text/plain; charset=UTF-8", body)
}

// SendHTML sends a single-part HTML email.
func (s *Sender) SendHTML(to, subject, htmlBody string) error {
	return s.sendMIME(to, subject, "text/html; charset=UTF-8", htmlBody)
}

// SendHTMLWithPlainAlt sends multipart/alternative with plain and HTML bodies.
func (s *Sender) SendHTMLWithPlainAlt(to, subject, plainBody, htmlBody string) error {
	if s.cfg.Host == "" || s.cfg.From == "" {
		return fmt.Errorf("smtp is not configured")
	}
	boundary := "hek-" + base64.StdEncoding.EncodeToString([]byte(subject))[:16]
...
func (s *Sender) sendMIME(to, subject, contentType, body string) error {
	if s.cfg.Host == "" || s.cfg.From == "" {
		return fmt.Errorf("smtp is not configured")
	}
